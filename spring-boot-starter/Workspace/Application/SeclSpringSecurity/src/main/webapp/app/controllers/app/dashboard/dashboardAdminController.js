...
define(['app'], function (app) {
	...
	var dashboardAdminController = function ($rootScope, $scope, $log, $timeout, $route, _,  messageService, dashboardService, $window, 
			$filter,constantService, localStorageService, configurationService,ngTableParams, ngProgress, comboService, wsClientService, loadService, reportsService, navigationService, constantJSON) {
		...
		$scope.title = {value: "Dashboard_DashboardAdmin"};
		...
		var loadBranchInformation = function () {
			...
			promisBranchInfo.then(function(data) {
				...
				$scope.dashboardInfo.branchID = $scope.branchInfos[0].branchID;
				$scope.searchParam.branchID = $scope.branchInfos[0].branchID;
			});
		...
		var init = function () {
			...
			if(userInfo.selectedLanguage == 'English'){
				$scope.columns = [
				                  ...
				                  //{ title: constantService.AccountIDEn, field: 'accountID', visible: true },
				                  { title: constantService.BranchNameEn, field: 'branchName', visible: true },
				                  { title: constantService.transactionTypeEn, field: 'transType', visible: true },
				                  { title: constantService.transactionDateEn, field: 'strTransDate', visible: true },
				                  { title: constantService.transAmountEn, field: 'transAmount', visible: true },
				                  { title: constantService.statusEn, field: 'transStatus', visible: true }
				                  ];
				...
				$scope.columns = [
				                  ...
				                  //{ title: constantService.AccountIDBn, field: 'accountID', visible: true },
				                  { title: constantService.BranchNameBn, field: 'branchName', visible: true },
				                  { title: constantService.transactionTypeBn, field: 'transType', visible: true },
				                  { title: constantService.transactionDateBn, field: 'strTransDate', visible: true },
				                  { title: constantService.transAmountBn, field: 'transAmount', visible: true },
				                  { title: constantService.statusBn, field: 'transStatus', visible: true }
				                  ];
	...
	app.register.controller('dashboardAdminController', ['$rootScope', '$scope', '$log', '$timeout', '$route', '_',  
	                                                     'messageService', 'dashboardService','$window','$filter', 'constantService', 'localStorageService',
	                                                     'configurationService','ngTableParams', 'ngProgress','comboService','wsClientService', 'loadService','reportsService','navigationService','constantJSON', dashboardAdminController]);
