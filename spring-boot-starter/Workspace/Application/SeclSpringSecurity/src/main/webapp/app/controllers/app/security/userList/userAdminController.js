...
define(['app'], function (app) {
	...
	var userAdminController = function ($rootScope, $scope, $log, $filter, $timeout, $route, _,  messageService, 
		...
		$scope.title = {value: "Security_AdminUser"};
		...
		$scope.roleList = constantJSON.adminUserRoleList;
		...
		var changeUserStatus = function (user){	
			...
			promis.then(function(data) {
				if (!data.success) {
					...
					if(data.message == "ACTIVE_BM_EXISTS_BY_BRANCH")
					{
		...
		//Card View Edit Option Service_Point_ViewDetails
		$scope.goForEdit = function(user) {
			navigationService.menuNavigation('Security_User_ViewDetails' + '/' + constantService.Update + '/' + user.loginId);        
		};
		//End Card View Edit Option

		$scope.goToNewUserPage = function() {
			navigationService.menuNavigation('Security_AdminUser' + '/' + constantService.Add + '/' + 'nan');
		};
		...
		var getUserList = function () {
			...
			promis = userService.userListForRoleAdmin(userObj);
			promis.then(function (data) {
				ngProgress.complete();
		...
		$scope.$on(constantService.LanguageChange, function (event, languageChange) {
		 ...
		 var getStatus = function(){
				$scope.statusList = constantJSON.docStatusAdmin;
				$scope.statusList = _.sortBy($scope.statusList, 'statusName');
			};
	...
	app.register.controller('userAdminController', ['$rootScope', '$scope', '$log', '$filter', '$timeout', 
	                                                ...
	                                                'loadService', 'comboService', 'constantJSON', userAdminController]);
