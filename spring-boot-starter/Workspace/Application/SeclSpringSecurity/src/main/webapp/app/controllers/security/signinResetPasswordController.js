...
define(['app'], function (app) {
	...
	var signinResetPasswordController = function ($rootScope, $scope, $log, $timeout, $route, $routeParams, _,  
			messageService, constantService, configurationService, navigationService, localStorageService, signInService, 
			ngProgress, loadService,passwordPolicyService) {
		...
		$scope.goToDashboard = function(user) {
			...
			var loginAgain = { 
					...
					operationType : constantService.userlogin
			};
			
			promis = signInService.doSignIn(loginAgain);
			...
			promis.then(function (data) {
				...
				if(userInfo.data.loginId != userInfo.data.changedBy && userInfo.data.reset)
				{
		...
		$scope.resetPassword = function (user){	
			...
			promis.then(function(data) {
				...
				messageService.showMessage(constantService.Success, 'PCS1000');

				$scope.user = {userID: $routeParams.oid};
				$timeout( function(){
					$scope.goToDashboard(user);
				}, 3000);
			});
		};
		...
		*****************************************************************/
		...
		var init = function (){
			...
			userInfo = localStorageService.getValue(configurationService.loginCookieStoreKey);
			if(userInfo.data.loginId == userInfo.data.changedBy)
			{
				navigationService.menuNavigation(userInfo.data.roleBean.menuJSON[0].leftmenuids[0]);
			}
			$scope.user = {userID: $routeParams.oid};
			getPasswordPolicy();
			ngProgress.complete();
	...
	app.register.controller('signinResetPasswordController', ['$rootScope', '$scope', '$log', '$timeout', '$route', '$routeParams',
	                                                          '_',  'messageService', 'constantService', 'configurationService', 'navigationService', 'localStorageService',
	                                                          'signInService', 'ngProgress', 'loadService','passwordPolicyService', signinResetPasswordController]);
