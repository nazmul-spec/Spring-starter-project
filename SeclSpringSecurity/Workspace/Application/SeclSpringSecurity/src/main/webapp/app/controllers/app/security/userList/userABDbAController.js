...
define(['app'], function (app) {
	...
	var userABDbAController = function ($rootScope, $scope, $log, $filter, $timeout, $route, _,  messageService, 
			comboService, userService, constantService,constantJSON, navigationService, localStorageService, configurationService, ngProgress, 
			modalService, ngTableParams, loadService ) {

		$scope.title = {value: "Security_ABDUser_BA"};
		...
		$scope.roleInfos = constantJSON.roleList;
		...
		$scope.goForEdit = function(user) {
			navigationService.menuNavigation('Security_ABDUser_BA' + '/' + constantService.Update + '/' + user.loginId);
		};
		//End Card View Edit Option

		$scope.goToNewUserPage = function() {
			navigationService.menuNavigation('Security_ABDUser_BA' + '/' + constantService.Add + '/' + 'nan');
		};
		...
		var getUserList = function () {
			...
			var userObj = { 
					operationType : constantService.GET_USER_LIST_FOR_ABD_USER,
		...
		$scope.$on(constantService.LanguageChange, function (event, languageChange) {
			...
			if(userInfo.selectedLanguage == 'English'){
				$scope.columns = [
				                  ...
				                  { title: constantService.CreatedByEn, field: 'createdBy', visible: true }
				                  ];
				...
				$scope.columns = [
				                  ...
				                  { title: constantService.CreatedByBn, field: 'createdBy', visible: true }
				                  ];
	...
	app.register.controller('userABDbAController', ['$rootScope', '$scope', '$log', '$filter', '$timeout', 
	                                             '$route', '_',  'messageService', 'comboService', 'userService', 'constantService','constantJSON', 'navigationService',
	                                             'localStorageService','configurationService', 'ngProgress', 'modalService', 'ngTableParams',
	                                             'loadService', userABDbAController]);
